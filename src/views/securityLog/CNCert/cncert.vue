...
export default {
  ...
  methods: {
    getNewData (page) {
      ...
      getTableData(baseURL + '/black_data', { page:page, source:'cert', current_time:formatTime })
        .then(res => {
          console.log(res)
  ...
  mounted () {
    const time = new Date()
    const formatTime = formatDate(time)
    getTableData(baseURL + '/black_data', { page:1, source:'cert', current_time:formatTime })
      .then(res => {
        console.log(res)
        this.tableData1 = res.data.black_data
        this.total = res.data.data_count
